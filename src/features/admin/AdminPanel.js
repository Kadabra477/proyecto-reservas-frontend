...
function AdminPanel() {
  ...
  const handleConfirmReserva = async (id) => {
    ...
    try {
      // Endpoint para confirmar reserva. Asegúrate que la ruta en el backend es `/api/reservas/{id}/confirmar`
      await api.put(`/reservas/${id}/confirmar`); // Corregido: sin `/admin/reservas`
  ...
  const handleDeleteReserva = async (id) => {
    if (window.confirm(`¿Estás seguro de eliminar la reserva con ID: ${id}?`)) {
      setMensaje({ text: '', type: '' });
      try {
        // Endpoint para eliminar reserva. Asegúrate que la ruta en el backend es `/api/reservas/{id}`
        await api.delete(`/reservas/${id}`); // Corregido: sin `/admin/reservas`
                ...
                <div className="admin-table-container"> {/* Contenedor para scroll horizontal */}
                    <table>
                        ...
                        {reservas.length > 0 ? reservas.map(r => (
                            ...
                            <td data-label="Cancha">{r.canchaNombre || 'N/A'}</td> {/* Usar canchaNombre del DTO */}
                            {/* Usar campo 'cliente' o combinar nombre/apellido */}
                            <td data-label="Cliente">{r.cliente || 'N/A'}</td> {/* 'cliente' es el campo combinado */}
                            <td data-label="Teléfono">{r.telefono || 'N/A'}</td>
                            {/* Formatear fecha y hora */}
                            <td data-label="Fecha y Hora">{formatDate(r.fechaHora)}</td> {/* Usar fechaHora del DTO */}
                            <td data-label="Estado">
