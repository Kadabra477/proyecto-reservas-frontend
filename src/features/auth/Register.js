...
import './Register.css';
import '../../styles/AuthForm.css'; // Estilos base

// Espera onGoToLogin como prop desde App.js
function Register({ onGoToLogin }) {
    const [nombreCompleto, setNombreCompleto] = useState(''); // Estado para nombre
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [confirmPassword, setConfirmPassword] = useState('');
    const [error, setError] = useState('');
    const [successMessage, setSuccessMessage] = useState(''); // Para mensaje de "revisa tu email"
    const [isLoading, setIsLoading] = useState(false);
    // No necesita useNavigate si usa onGoToLogin
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Llamada a la API de registro incluyendo nombreCompleto
            await api.post('/auth/register', {
                nombreCompleto: nombreCompleto.trim(),
                username: email, // email es el username
                password: password,
            });

            // Éxito: Mostrar mensaje sobre validación por email
            ...
            } else if (err.response?.status === 409) { // Conflicto (email ya existe)
        ...
        <div className="auth-background register-background">
            <div className="auth-container">
                <form onSubmit={handleSubmit} className="auth-form register-form" autoComplete="off">
                    ...
                    {successMessage && !error && <p className="auth-message success" style={{ marginBottom: '1.5em' }}>{successMessage}</p>}

                    {/* Ocultar formulario si el registro fue exitoso */}
                    {!successMessage && (
                        <>
                    ...
                    {/* Botón Google */}
                    {/* Ocultarlo si el registro fue exitoso? */}
                    {/* Botón Google */}
