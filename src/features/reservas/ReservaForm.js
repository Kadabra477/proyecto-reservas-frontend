...
function ReservaForm() {
    ...
    // --- Efecto para cargar los detalles de la cancha (sin cambios) ---
    const fetchCanchaDetails = useCallback(async () => {
        ...
        try {
            ...
            if (!response.data) {
                ...
                setCancha(response.data); // Guardamos info aunque no disponible
            } else {
    ...
    // --- Manejador de cambio en formulario (sin cambios) ---
    ...
    const validarFormulario = () => {
        ...
        const { nombre, apellido, dni, telefono, fecha, hora } = formulario;

        if (!nombre.trim() || !apellido.trim() || !dni.trim() || !telefono.trim() || !fecha || !hora) {
            setMensaje({ type: 'error', text: 'Todos los campos son obligatorios.' });
            return false;
        }
        if (!/^\d{7,8}$/.test(dni.trim())) {
            setMensaje({ type: 'error', text: 'El DNI debe contener 8 números (sin puntos).' });
            return false;
        }
    ...
    // --- Manejador del envío del formulario (MODIFICADO PARA PAGO) ---
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormulario({ nombre: '', apellido: '', dni: '', telefono: '', fecha: '', hora: '' });

            // 2. Crear la preferencia de pago en Mercado Pago
            setIsCreatingPreference(true); // Indicar que estamos creando la preferencia
            try {
                const preferenciaResponse = await api.post(`/pagos/crear-preferencia`, null, {
                    params: {
                        reservaId: reservaCreada.id,
                        pagador: `${reservaCreada.nombre} ${reservaCreada.apellido}`,
                        monto: cancha.precio || 2000, // Asumimos que el precio está en la cancha o ponelo fijo por ahora
                    }
                });                
                ...
                window.location.href = initPoint; // Redirección del navegador
                 ...
                 setMensaje({
                     type: 'error',
                     text: `La reserva (ID: ${reservaCreada.id}) se creó, pero no pudimos iniciar el pago con Mercado Pago. Por favor, contacta soporte o intenta pagar desde 'Mis Reservas'.`
                 });
    ...
    return (
        <div className="reserva-form-container">
            ...
            <h3 className="reserva-form-subtitle">Completa tus datos para reservar</h3>
            <form onSubmit={handleSubmit} className="reserva-formulario" noValidate>
                ...
                <button type="submit" className="reserva-submit-button" disabled={isSubmitting || isCreatingPreference || !cancha.disponible}>
                    {isSubmitting ? (isCreatingPreference ? 'Iniciando Pago...' : 'Creando Reserva...') : 'Reservar y Pagar'}
                </button>
            </form>
